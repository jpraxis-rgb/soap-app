...
import {
  ...
  TOPICS,
  type TopicManifest,
} from '../content/disciplines/direito-constitucional/index.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DISCIPLINE_DIR = path.resolve(__dirname, '../content/disciplines/direito-constitucional');
...
export async function seedDireitoConstitucional(): Promise<{ inserted: number; topics: number }> {
  let inserted = 0;
...
if (invokedDirectly) {
  seedDireitoConstitucional()
    .then((r) => {
      console.log(`✅ Direito Constitucional: ${r.inserted} items across ${r.topics} topics`);
      process.exit(0);
    })
    .catch((err) => {
