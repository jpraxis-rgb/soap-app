...
import {
  ...
  TOPICS,
  type TopicManifest,
} from '../content/disciplines/direito-administrativo/index.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DISCIPLINE_DIR = path.resolve(__dirname, '../content/disciplines/direito-administrativo');
...
export async function seedDireitoAdministrativo(): Promise<{ inserted: number; topics: number }> {
  let inserted = 0;
...
if (invokedDirectly) {
  seedDireitoAdministrativo()
    .then((r) => {
      console.log(`✅ Direito Administrativo: ${r.inserted} items across ${r.topics} topics`);
      process.exit(0);
    })
    .catch((err) => {
