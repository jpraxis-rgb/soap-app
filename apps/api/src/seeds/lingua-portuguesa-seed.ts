...
import {
  ...
  TOPICS,
  type TopicManifest,
} from '../content/disciplines/lingua-portuguesa/index.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DISCIPLINE_DIR = path.resolve(__dirname, '../content/disciplines/lingua-portuguesa');
...
export async function seedLinguaPortuguesa(): Promise<{ inserted: number; topics: number }> {
  let inserted = 0;
...
if (invokedDirectly) {
  seedLinguaPortuguesa()
    .then((r) => {
      console.log(`✅ Língua Portuguesa: ${r.inserted} items across ${r.topics} topics`);
      process.exit(0);
    })
    .catch((err) => {
